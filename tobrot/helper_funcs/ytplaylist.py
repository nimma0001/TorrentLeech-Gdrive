...
async def yt_playlist_downg(message, i_m_sefg, G_DRIVE):
    ...
    try:
        ...
        pass
    cmd = ["youtube-dl", "--merge-output-format", "mp4", "--postprocessor-args", "-id3v2_version 3", "-i", "-f", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best", "--embed-thumbnail", "--write-thumbnail", "--write-all-thumbnails", "--add-metadata", "-o", f"{fol_der}/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s", f"{url}"]
