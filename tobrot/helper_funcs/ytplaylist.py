...
async def yt_playlist_downg(message, i_m_sefg, G_DRIVE):
    ...
    try:
        ...
        pass
    cmd = ["youtube-dl", "--merge-output-format", "mp4", "-i", "-f", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best", "--embed-thumbnail", "--add-metadata", "-o", f"{fol_der}/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s", f"{url}"]
